...
class RampEngine {
  ...
  _motorSet(motor, speed, label, trainId) {
    ...
    if (speed !== 0) {
      // Движение: предпочтительно setPower, fallback setSpeed
      if (typeof motor.setPower === "function") {
      ...
      // Остановка: brake() — это 0x7F (рекомендуется), fallback setPower(0)
      if (typeof motor.brake === "function") {
        this._dbg(`${label}: brake() [0x7F]`, trainId);
        safeCall(motor.brake);
      } else if (typeof motor.setPower === "function") {
        this._dbg(`${label}: setPower(0) [FLOAT fallback]`, trainId);
