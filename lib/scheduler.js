 ...
 *
 * 400 мс — компромисс:
 *   - При стоянке на метке: блокирует повторный trigger (метка читается ~10 раз/сек)
 *   - При движении: поезд над меткой ~100-200 мс, следующий проезд через несколько секунд
 * 800 мс было избыточно — могло пропустить быстрый проезд на высокой скорости.
 * @constant
 */
const COLOR_COOLDOWN_MS = 400;
