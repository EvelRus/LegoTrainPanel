...
// Используется для оффлайн-режима, кэширования и push-уведомлений
...
// Показываем тултип при наведении на элемент с data-tip
...
// Двигаем тултип за курсором с небольшим смещением
document.addEventListener("mousemove", (e) => {
  let x = e.clientX + 14;
  let y = e.clientY - 38;

  // Не даём тултипу вылезти за правый край экрана
  if (x + tooltipEl.offsetWidth > window.innerWidth - 8) {
    x = e.clientX - tooltipEl.offsetWidth - 14;
  }
  // Не даём вылезти за верхний край
...
// Скрываем при уходе курсора с элемента с data-tip
document.addEventListener("mouseout", (e) => {
  if (!e.target.closest("[data-tip]")) tooltipEl.style.display = "none";
});

/* ─────────────────────────────────────────────────────────────────────
   SPEEDOMETER — круговой спидометр-шкала
   ───────────────────────────────────────────────────────────────────── */

const S = { CX: 90, CY: 80, R: 72 }; // Константы центра и радиуса

/**
 * Преобразование угла и радиуса в координаты на плоскости
 * Угол отсчитывается от вертикали вверх (0° = вверх)
 */
function polarSpd(a, r) {
  const rad = (a * Math.PI) / 180;
  return {
    x: S.CX + r * Math.sin(rad),
    y: S.CY - r * Math.cos(rad),
  };
}
...
/**
 * Генерирует SVG-код спидометра для конкретного поезда
 * @param {string} id — идентификатор поезда
 * @returns {string} готовый SVG
 */
function buildSpeedo(id) {
  let ticks = "";
  let lbls = "";

  // Деления шкалы каждые 18°
  for (let a = -90; a <= 90; a += 18) {
    const i = polarSpd(a, S.R - 6);
    const o = polarSpd(a, S.R + 2);
    ticks += `<line x1="${f(i.x)}" y1="${f(i.y)}" x2="${f(o.x)}" y2="${f(o.y)}" stroke="#2a2d38" stroke-width="1.5"/>`;
  }

  // Основные подписи скоростей
  [
    [-90, "−100"],
    [-45, "−50"],
    [0, "0"],
    ...
    const p = polarSpd(a, S.R + 13);
    const anc = a < -20 ? "start" : a > 20 ? "end" : "middle";
    lbls += `<text x="${f(p.x)}" y="${f(p.y)}" text-anchor="${anc}" fill="#3a3e4e" font-family="'Share Tech Mono',monospace" font-size="7">${t}</text>`;
  });

  return `
    <svg class="speedo-svg" viewBox="0 0 180 90" id="speedo-${id}">
      <defs>
        <linearGradient id="sg${id}" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%"   stop-color="#40c4ff"/>
          <stop offset="50%"  stop-color="#00e676"/>
          <stop offset="75%"  stop-color="#ffd500"/>
          <stop offset="100%" stop-color="#ff3b3b"/>
        </linearGradient>
      </defs>
      <path d="M 18,80 A 72,72 0 1,1 162,80" fill="none" stroke="#252830" stroke-width="9" stroke-linecap="round"/>
      <path d="M 18,80 A 72,72 0 1,1 162,80" fill="none" stroke="url(#sg${id})" stroke-width="9" stroke-linecap="round" opacity=".24"/>
      ${ticks}
      ${lbls}
      <path id="sarc-${id}" fill="none" stroke-width="9" stroke-linecap="round" opacity=".88"/>
      <line id="sneedle-${id}" x1="${S.CX}" y1="${S.CY}" x2="${S.CX}" y2="${S.CY - S.R + 5}" stroke="#fff" stroke-width="2.2" stroke-linecap="round" style="transform-origin:${S.CX}px ${S.CY}px;transform:rotate(0deg);transition:transform .28s cubic-bezier(.25,.8,.25,1)"/>
      <circle cx="${S.CX}" cy="${S.CY}" r="4.5" fill="#fff" opacity=".88"/>
      <circle cx="${S.CX}" cy="${S.CY}" r="2" fill="#0d0d0f"/>
    </svg>`;
}

/**
 * Обновляет положение стрелки и дугу активной скорости
 * @param {string} id — id поезда
 * @param {number} speed — скорость от -100 до +100
 */
function updateSpeedo(id, speed) {
  const needle = document.getElementById(`sneedle-${id}`);
  const arcEl = document.getElementById(`sarc-${id}`);
  if (!needle) return;
  ...
  const tipX = f(S.CX + tipR * Math.sin(aRad));
  const tipY = f(S.CY - tipR * Math.cos(aRad));
  const sweep = speed > 0 ? 1 : 0;

  arcEl.setAttribute(
    "d",
    `M ${f(S.CX)},${f(S.CY - tipR)} A ${tipR},${tipR} 0 0,${sweep} ${tipX},${tipY}`,
  );
  arcEl.setAttribute("stroke", color);
}

/* ─────────────────────────────────────────────────────────────────────
   SOCKET.IO — основное соединение с сервером
   ───────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────
   Человекочитаемые имена устройств (маппинг из node-poweredup)
   ───────────────────────────────────────────────────────────────────── */

const DEVICE_TYPE_NAMES = {
  TrainMotor: "Поездной мотор",
  TrainMotorLarge: "Поездной мотор (L)",
  TrainMotorSmall: "Поездной мотор (S)",
  MediumMotor: "Мотор M",
  LargeMotor: "Мотор L",
  XLargeMotor: "Мотор XL",
  TechnicLargeMotor: "Technic мотор L",
  TechnicXLargeMotor: "Technic мотор XL",
  ColorDistanceSensor: "Датчик цвет+дист.",
  ColorSensor: "Датчик цвета",
  DistanceSensor: "Датчик расстояния",
  TechnicColorSensor: "Technic датчик цвета",
  TechnicDistanceSensor: "Technic датчик дист.",
  DCMotor: "DC-мотор",
  "—": "—",
};

function friendlyDeviceName(raw) {
  return DEVICE_TYPE_NAMES[raw] || raw || "—";
}

/* ─────────────────────────────────────────────────────────────────────
   Бейджи портов на карточке поезда
   ───────────────────────────────────────────────────────────────────── */

const PORT_ICONS = { motor: "⚙", sensor: "📡" };

const PORT_SHORT = {
  TrainMotorLarge: "TrnMtr",
  TrainMotor: "TrnMtr",
  TrainMotorSmall: "TrnSml",
  MediumMotor: "MedMtr",
  LargeMotor: "LrgMtr",
  XLargeMotor: "XLMtr",
  TechnicLargeMotor: "TechMtr",
  TechnicXLargeMotor: "TechXL",
  ColorDistanceSensor: "Clr+Dst",
  ColorSensor: "Clr",
  DistanceSensor: "Dst",
  TechnicColorSensor: "TColr",
  TechnicDistanceSensor: "TDst",
  DCMotor: "DC",
};

/**
 * Обновляет бейджи портов на карточке поезда
 */
function updatePortBadges(id, ports) {
  ...
  const badges = Object.entries(ports)
    ...
    .map(([port, info]) => {
      ...
      const friendlyName = friendlyDeviceName(info.name);
      return `<div class="port-badge ${cls}" data-tip="${port}: ${friendlyName}">${ico} ${port}·${short}</div>`;
    });
...
/* ─────────────────────────────────────────────────────────────────────
   Основные события от сервера через Socket.IO
   ───────────────────────────────────────────────────────────────────── */
...
socket.on("consistsUpdate", (d) => {
  consists = d;
  renderConsistList();
  renderSchActionParams();
  trainIds.forEach((id) => renderConsistBadge(id));
});

socket.on("scenariosUpdate", (d) => {
  scenarios = d;
  renderScenarioList();
  renderSchActionParams();
});

socket.on("schedulesUpdate", (d) => {
  schedules = d;
  renderScheduleList();
});

socket.on("trafficLightsUpdate", (d) => {
  trafficLights = d;
  renderTrafficLightList();
});
...
socket.on("newTrain", (d) => {
  addTrainCard(d);
  updatePortBadges(d.id, d.ports);
...
socket.on("sensorUpdate", ({ trainId, type, color, colorName, distance }) => {
  ...
  if (type === "color" || type === "colorAndDistance") {
    ...
    if (dot) {
      ...
      dot.style.boxShadow = css ? `0 0 5px ${css}99` : "none";
    }
    if (lbl) lbl.textContent = colorName ?? "?";
    if (distance != null) {
      const ds = $(`sdist-${trainId}`);
      if (ds) ds.textContent = `${distance} мм`;
    }
  }
  if (type === "distance") {
    const ds = $(`sdist-${trainId}`);
    if (ds) ds.textContent = distance != null ? `${distance} мм` : "— мм";
  }
});
...
/* ─────────────────────────────────────────────────────────────────────
   Добавление / удаление карточки поезда в интерфейсе
   ───────────────────────────────────────────────────────────────────── */
...
function addTrainCard(d) {
  ...
  const friendlyMotor = friendlyDeviceName(deviceTypeName);
  const friendlySensor = sensorTypeName
    ? friendlyDeviceName(sensorTypeName)
    : null;
  const infoTip = `🔧 Прошивка: ${firmwareVersion}\n⚙ Железо: ${hardwareVersion}\n📡 Хаб: ${hubTypeName}${
    friendlyMotor !== "—" ? `\n⚙ Мотор: ${friendlyMotor}` : ""
  }${friendlySensor ? `\n📡 Датчик: ${friendlySensor}` : ""}`;

  const presetsHtml = presets
    .map(
      (v) =>
        `<button class="btn btn-preset" onclick="setSpeed('${esc(id)}',${v})">⚡${v}%</button>`,
    )
    .join("");

  const sensorHtml = sensorPort
    ? `<div class="sensor-row" id="sensor-${id}">
         <span class="s-label">📡 ${sensorPort}</span>
         <div class="sensor-color-dot" id="scolor-${id}"></div>
         <span id="scolorname-${id}" class="s-colorname">—</span>
         <span id="sdist-${id}" class="s-dist">— мм</span>
       </div>`
    : "";
  ...
  card.innerHTML = `
    <div class="dc-overlay">
      <p>⚠ НЕТ СВЯЗИ</p>
      <button class="btn-reconnect" id="rc-${id}" onclick="requestReconnect('${esc(id)}')">⟳ ПЕРЕПОДКЛЮЧИТЬ</button>
    </div>
    <div class="card-inner">
      ...
      <div class="photo-area${photo ? " has-photo" : ""}" id="photo-${id}">
        ${photo ? `<img src="${photo}" alt="">` : ""}
      </div>

      <div class="card-body">
        <div class="speed-row">
          <div class="sr-speed" id="srspeed-${id}">
            <div class="speed-num stop" id="speed-${id}">0</div>
            <div class="speed-unit">% POWER</div>
          </div>

          <div class="sr-center">
            ${sensorHtml}
            <div class="sound-row">
              <button class="btn-snd${sounds.start ? "" : " na"}" onclick="playOnce('${esc(id)}','start')" data-tip="Звук старта">
                <span class="ico">🚀</span><span class="snd-lbl">СТАРТ</span>
              </button>
              <button class="btn-snd${sounds.horn ? "" : " na"}" id="horn-${id}" 
                onmousedown="hornOn('${esc(id)}')" onmouseup="hornOff('${esc(id)}')"
                ontouchstart="hornOn('${esc(id)}')" ontouchend="hornOff('${esc(id)}')" data-tip="Гудок (удерж.)">
                <span class="ico">📯</span><span class="snd-lbl">ГУДОК</span>
              </button>
              <button class="btn-snd${sounds.stop ? "" : " na"}" onclick="playOnce('${esc(id)}','stop')" data-tip="Звук торможения">
                <span class="ico">🛑</span><span class="snd-lbl">ТОРМОЗ</span>
              </button>
            </div>
          </div>

          <div class="dir-badge" id="dir-${id}">
            <div class="arr">⏹</div><div>СТОП</div>
          </div>
        </div>
        ...
        <div class="actions">
          <div class="speed-btns">
            ...
            <button class="btn" onclick="deltaSpeed('${esc(id)}',-20)" data-tip="−20% (минимум ±20)">−20</button>
  ...
  // Debounced отправка скорости со слайдера
  const sl = $(`slider-${id}`);
  let deb,
    lastEmitted = speed;
...
/* ─────────────────────────────────────────────────────────────────────
   Звуковое сопровождение поездов
   ───────────────────────────────────────────────────────────────────── */
...
function reloadSounds(id, sounds) {
  stopLoop(id);
  initSounds(id, sounds);
}
...
/** Запуск зацикленного звука движения */
...
/** Установка громкости зацикленного звука в зависимости от скорости */
...
/**
 * Логика переключения звуков при изменении скорости
 * Важно: учитывает переход 0 → движение, движение → 0, смена направления
 */
function handleSpeedChange(id, newSpeed) {
  ...
  if (prev === 0 && newSpeed !== 0) {
    ...
    if (s.stop) {
      s.stop.pause();
      s.stop.currentTime = 0;
    }
    stopLoop(id);

    if (!s.start || globalMuted) {
      startLoop(id);
    ...
    s.start.pause();
    s.start.currentTime = 0;
    s.start.play().catch(() => {});

    const doStartLoop = () => {
      clearTimeout(_startLoopTimer[id]);
      delete _startLoopTimer[id];
      s.start.removeEventListener("ended", onStartEnded);
      startLoop(id);
      setLoopVol(id, prevSpeed[id] ?? newSpeed);
    };

    const onStartEnded = () => doStartLoop();
    s.start.addEventListener("ended", onStartEnded, { once: true });

    // Fallback на таймер, если событие ended не сработало
    const durMs =
      s.start.duration && isFinite(s.start.duration) && s.start.duration > 0.1
        ? s.start.duration * 1000 + 150
        : 1200;
    _startLoopTimer[id] = setTimeout(doStartLoop, durMs);
    ...
    if (s.start) {
      s.start.removeEventListener("ended", () => {});
      s.start.pause();
      s.start.currentTime = 0;
    }
    ...
    if (!globalMuted) playOnce(id, "stop");
  } else if (prev !== 0 && newSpeed !== 0) {
    // Смена скорости / направления → только громкость
    setLoopVol(id, newSpeed);
  }
}
...
/* ─────────────────────────────────────────────────────────────────────
   Управление скоростью поезда (кнопки, слайдер, дельта)
   ───────────────────────────────────────────────────────────────────── */

function setSpeed(id, speed) {
  socket.emit("setSpeed", { trainId: id, speed });
}

const MIN_SPEED = 20; // должна совпадать с MIN_MOVE_POWER на сервере

/**
 * Изменение скорости на delta с учётом мёртвой зоны около нуля
 */
function deltaSpeed(id, d) {
  const sl = $(`slider-${id}`);
  if (!sl) return;
  const cur = +sl.value;
  let next;
  ...
  if (cur === 0) {
    next = d > 0 ? MIN_SPEED : -MIN_SPEED;
  } else {
    next = cur + d;
    if (next > 0 && next < MIN_SPEED) next = MIN_SPEED;
    if (next < 0 && next > -MIN_SPEED) next = -MIN_SPEED;
  }
...
function requestReconnect(id) {
  socket.emit("reconnectHub", { trainId: id });
}
...
function setConnected(id, ok) {
  ...
  const rc = $(`rc-${id}`);
  if (rc && ok) {
    rc.classList.remove("trying");
    rc.textContent = "⟳ ПЕРЕПОДКЛЮЧИТЬ";
  }
}

function batCls(l) {
  return l > 40 ? "hi" : l > 15 ? "mid" : "lo";
}
...
function addLogEntry(entry) {
  ...
  el.innerHTML = `
    ...
    <span class="log-lvl lv-${entry.level}">${entry.level}</span>
    <span class="log-msg">
      ${entry.trainId ? `<b>${entry.trainId.slice(0, 8)}</b> ` : ""}
      ${entry.message}
    </span>`;
...
async function loadLogFile() {
  ...
  try {
    const text = await fetch(
      `/api/logs/file?name=${encodeURIComponent(name)}`,
    ).then((r) => r.text());
    $("logList").innerHTML = "";
    text
      .split("\n")
      ...
      .forEach((line) => {
        ...
        if (m) {
          el.innerHTML =
            `<span class="log-ts">${m[1].replace("T", " ").replace(/\.\d+Z$/, "")}</span>` +
...
/* ─────────────────────────────────────────────────────────────────────
   Модальное окно «Автоматика» (составы, сценарии, расписания, светофоры)
   ───────────────────────────────────────────────────────────────────── */
...
function closeAutoModal() {
  $("autoBackdrop").classList.remove("open");
}

function closeAutoOuter(e) {
  if (e.target === $("autoBackdrop")) closeAutoModal();
}

function setAutoTab(tab) {
  ["consists", "scenarios", "schedules", "trafficlights"].forEach((t) => {
    $(`aTab-${t}`)?.classList.toggle("active", t === tab);
    $(`aPanel-${t}`)?.classList.toggle("active", t === tab);
  });
}

function setScTab(tab) {
  document
    .querySelectorAll(".sc-stab")
    .forEach(
      (b) =>
        (b.className = "sc-stab" + (b.dataset.tab === tab ? " active" : "")),
    );
  document
    .querySelectorAll(".sc-subpanel")
    .forEach(
      (p) =>
        (p.className = "sc-subpanel" + (p.id === `sc-${tab}` ? " active" : "")),
    );
}
...
function renderConsistTrainList() {
  ...
  el.innerHTML =
    [...trainIds]
      .map((id) => {
        const n =
          $(`card-${id}`)?.querySelector(".train-name")?.textContent || id;
        return `<label class="train-check-item"><input type="checkbox" id="cc-${id}" value="${id}"> ${n}</label>`;
      })
      .join("") ||
    '<div style="color:var(--mut);font-family:var(--fm);font-size:.7rem">Нет подключённых поездов</div>';
}

function createConsist() {
  const name = $("cName").value.trim();
  if (!name) return;
  const ids = [...$("cTrainList").querySelectorAll("input:checked")].map(
    (c) => c.value,
  );
  if (!ids.length) return;
  socket.emit("saveConsist", {
    id: `c${Date.now()}`,
    consist: { name, trainIds: ids, speed: 0 },
  });
  $("cName").value = "";
  $("cTrainList")
...
function renderConsistList() {
  ...
  if (!entries.length) {
    el.innerHTML =
      '<div style="color:var(--mut);font-family:var(--fm);font-size:.7rem;padding:8px">Составов нет</div>';
    return;
  }
...
function consistSpeedDisplay(id, v) {
  const el = $(`cspd-${id}`);
  if (el) el.textContent = `${v}%`;
}

function setConsistSpeed(id, speed) {
  socket.emit("setConsistSpeed", { consistId: id, speed });
}

function renderConsistBadge(trainId) {
  const badge = $(`cbadge-${trainId}`);
  if (!badge) return;
  const inC = Object.values(consists).some((c) =>
    (c.trainIds || []).includes(trainId),
  );
...
/* ─────────────────────────────────────────────────────────────────────
   РУЧНОЙ ПОСТРОИТЕЛЬ СЦЕНАРИЕВ (Manual Scenario Builder)
   ───────────────────────────────────────────────────────────────────── */
...
function updateMsCondition() {
  const type = $("msConditionType").value;
  const params = $("msConditionParams");
  if (!params) return;
  ...
  if (type === "colorSensor") {
    ...
    params.innerHTML = `
      ...
      <label style="display:flex;align-items:center;gap:6px;margin-top:6px;font-family:var(--fm);font-size:.65rem;cursor:pointer">
        <input type="checkbox" id="msCondNoTimeout">
        <span style="color:var(--mut)">Без таймаута — ждать цвет бесконечно</span>
      </label>
      <div style="font-family:var(--fm);font-size:.61rem;color:var(--mut);margin-top:4px">
        Шаг выполнится когда датчик зафиксирует этот цвет.
        Длительность — таймаут (если галочка не стоит).
      </div>`;
...
function addManualStep() {
  ...
  const trainName =
    trainSelect.options[trainSelect.selectedIndex]?.text || trainId;
  const speed = +$("msSpeed").value;
  const duration = Math.max(0, +$("msDuration").value || 3);
  ...
  if (condType === "colorSensor") {
    ...
    noTimeout = $("msCondNoTimeout")?.checked ?? false;
  }
...
function loadScenarioToBuilder(name) {
  ...
  manualSteps = sc.steps.map((step, i) => {
    ...
    const duration =
      timeout > 0
        ? Math.max(0, Math.round(timeout / 100) / 10)
        : Math.max(0, Math.round((nextDelay - step.delay) / 100) / 10);

    const trainName =
      document.querySelector(`#card-${step.trainId} .train-name`)
        ?.textContent || step.trainId;
    ...
    return {
      ...
      speed: step.speed ?? 0,
      duration,
      noTimeout: timeout === 0 && !!step.condition,
...
function editManualStep(i) {
  ...
  const trainOpts =
    [...trainIds]
      .map((id) => {
        const n =
          document.querySelector(`#card-${id} .train-name`)?.textContent || id;
        return `<option value="${id}" ${id === step.trainId ? "selected" : ""}>${n}</option>`;
      })
      .join("") ||
    `<option value="${step.trainId}" selected>${step.trainName}</option>`;
  ...
  el.innerHTML = `
    ...
    <div class="step-info" style="flex:1;display:grid;gap:5px">
      ...
      <div style="display:grid;grid-template-columns:auto 1fr auto auto;gap:5px;align-items:center">
        ...
        <input type="number" class="field-input" id="es-dur-${i}" min="0" max="3600" step="0.5" value="${step.duration}" style="padding:2px 5px" />
...
function applyStepEdit(i) {
  ...
  step.trainName =
    trainSel?.options[trainSel.selectedIndex]?.text || step.trainId;
  step.speed = +($(`es-speed-${i}`)?.value ?? step.speed);
  step.duration = Math.max(0, +($(`es-dur-${i}`)?.value ?? step.duration));
  renderManualSteps();
}
...
function renderManualSteps() {
  ...
  if (!manualSteps.length) {
    el.innerHTML =
      '<div style="color:var(--mut);font-family:var(--fm);font-size:.7rem;padding:6px 0">Шагов нет — настройте параметры и нажмите «Добавить шаг»</div>';
    return;
  }
  ...
  el.innerHTML =
    manualSteps
      .map((step, i) => {
        ...
        const durLbl = step.duration > 0 ? `${step.duration}с` : "∞";

        const condHtml = step.condition
          ? `<div class="step-cond">⚡ ждём: цвет порт ${step.condition.port} = ${COLOR_NAMES_RU[step.condition.color] ?? step.condition.color}${step.noTimeout ? " (без таймаута)" : ` (таймаут ${durLbl})`}</div>`
          : "";

        return `
          <div class="step-card" id="ms-step-${i}">
            <div class="step-num">${i + 1}</div>
            <div class="step-info">
              <span class="step-train">${step.trainName}</span>
              <span style="color:var(--mut)"> → </span>
              <span class="${speedCls}">${speedLbl}</span>
              <span class="step-dur"> · ${durLbl} (t+${at}с)</span>
              ${condHtml}
            </div>
...
function saveManualScenario() {
  ...
  for (const step of manualSteps) {
    const timeout =
      step.condition && step.noTimeout ? 0 : Math.round(step.duration * 1000);
  ...
  socket.emit("saveScenario", {
    name,
    data: { steps, created: new Date().toISOString(), source: "manual" },
  });
...
function renderScenarioList() {
  ...
  if (!entries.length) {
    el.innerHTML =
      '<div style="color:var(--mut);font-family:var(--fm);font-size:.7rem;padding:8px">Сценариев нет</div>';
    return;
  }
  el.innerHTML = entries
    .map(([name, sc]) => {
...
/* ─────────────────────────────────────────────────────────────────────
   РАСПИСАНИЕ (Schedules) — задачи по времени
   ───────────────────────────────────────────────────────────────────── */

function renderSchActionParams() {
  const type = $("schActionType")?.value;
  const el = $("schActionParams");
  if (!el) return;
  ...
  if (type === "setSpeed") {
    ...
    el.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
        <div>
          <div class="field-label">Поезд</div>
          <select class="field-input" id="schTrain">${opts || "<option>—</option>"}</select>
        </div>
        <div>
          <div class="field-label">Скорость %</div>
          <input class="field-input" id="schSpeed" type="number" min="-100" max="100" value="50">
        </div>
      </div>`;
  } else if (type === "playScenario") {
    const opts = Object.keys(scenarios)
      .map((n) => `<option value="${n}">${n}</option>`)
      .join("");
    el.innerHTML = `
      <div>
        <div class="field-label">Сценарий</div>
        <select class="field-input" id="schScenario">${opts || "<option>—</option>"}</select>
      </div>`;
  } else if (type === "setConsist") {
    const opts = Object.entries(consists)
    ...
    el.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
        <div>
          ...
          <select class="field-input" id="schConsist">${opts || "<option>—</option>"}</select>
        </div>
        <div>
          <div class="field-label">Скорость %</div>
          <input class="field-input" id="schCSpeed" type="number" min="-100" max="100" value="50">
        </div>
      </div>`;
  } else {
    el.innerHTML = "";
...
function addSchedule() {
  ...
  if (!time) return;
  const days = [...document.querySelectorAll(".day-btn.on")].map(
    (b) => +b.dataset.d,
  );
  const type = $("schActionType").value;
  ...
  if (type === "setSpeed") {
    action.trainId = $("schTrain")?.value;
    action.speed = +($("schSpeed")?.value || 50);
  }
  if (type === "playScenario") {
    action.name = $("schScenario")?.value;
  }
  if (type === "setConsist") {
    action.consistId = $("schConsist")?.value;
    action.speed = +($("schCSpeed")?.value || 50);
  }

  socket.emit("addSchedule", {
    id: `s${Date.now()}`,
    schedule: { name, time, days, action },
  });

  $("schName").value = "";
  $("schTime").value = "";
  document
    .querySelectorAll(".day-btn.on")
...
function renderScheduleList() {
  ...
  if (!entries.length) {
    el.innerHTML =
      '<div style="color:var(--mut);font-family:var(--fm);font-size:.7rem;padding:8px">Задач нет</div>';
    return;
  }
  el.innerHTML = entries
    .map(([id, s]) => {
      ...
      const act =
        s.action?.type === "estop"
          ? "⛔ E-STOP"
          : s.action?.type === "setSpeed"
            ? `→ ${(s.action.trainId || "").slice(0, 8)} ${s.action.speed}%`
            : s.action?.type === "playScenario"
              ? `▶ ${s.action.name}`
              : s.action?.type === "setConsist"
                ? `🔗 ${s.action.speed}%`
                : "—";
...
async function openModal(id) {
  ...
  const entry = currentConfig[id] || {};
  const sounds = entry.sounds || {};
  const presets = entry.presets || [20, 50, 80];
  ...
  const step = entry.rampStepSize || 10;
  const ms = entry.rampStepMs || 100;
  ...
  // Зажимаем пресеты в допустимом диапазоне
  $("mPre0").value = Math.max(-100, Math.min(100, presets[0] ?? 20));
  $("mPre1").value = Math.max(-100, Math.min(100, presets[1] ?? 50));
  $("mPre2").value = Math.max(-100, Math.min(100, presets[2] ?? 80));

  document
    .querySelectorAll(".file-browser")
    .forEach((fb) => fb.classList.remove("open"));
...
function updateRampHint() {
  const step = +$("mRampStep").value;
  const ms = +$("mRampMs").value;
  $("rStepVal").textContent = step;
  $("rMsVal").textContent = ms;
  $("rampHint").textContent =
    `Диапазон 200% за ~${(((200 / step) * ms) / 1000).toFixed(1)} с`;
}
...
async function saveModal() {
  ...
  const photo = $("mPhoto").value.trim();
  const start = $("mStart").value.trim();
  const stop = $("mStop").value.trim();
  const horn = $("mHorn").value.trim();
  const moving = $("mMoving").value.trim();
  const rampStepSize = +$("mRampStep").value;
  const rampStepMs = +$("mRampMs").value;

  const presets = [
    Math.max(-100, Math.min(100, +$("mPre0").value || 20)),
    Math.max(-100, Math.min(100, +$("mPre1").value || 50)),
    Math.max(-100, Math.min(100, +$("mPre2").value || 80)),
  ];

  if (!currentConfig[editingUuid]) currentConfig[editingUuid] = {};

  Object.assign(currentConfig[editingUuid], {
    name,
    photo,
  ...
  if (resp.ok) {
    ...
    if (pa) {
      if (photo) {
        pa.className = "photo-area has-photo";
        pa.innerHTML = `<img src="${photo}?t=${Date.now()}" alt="">`;
      } else {
    ...
    socket.emit("setRampParams", {
      trainId: editingUuid,
      stepSize: rampStepSize,
      stepMs: rampStepMs,
    });
    ...
    if (presetRow) {
      presetRow.innerHTML = presets
        .map(
          (v) =>
            `<button class="btn btn-preset" onclick="setSpeed('${esc(editingUuid)}',${v})">⚡${v}%</button>`,
        )
        .join("");
    }
...
/* ─────────────────────────────────────────────────────────────────────
   ПРЕВЬЮ АУДИО И ФОТО В НАСТРОЙКАХ
   ───────────────────────────────────────────────────────────────────── */
...
function togglePreview(fieldId) {
  if (previewFieldId === fieldId && previewAudio) {
    stopPreviewAudio();
    return;
  }
...
async function renderBrowser(fieldId, dirPath) {
  ...
  const pathHtml =
    `<button onclick="renderBrowser('${fieldId}','')">📁 public</button>` +
    parts
      .map((p, i) => {
        const a = parts.slice(0, i + 1).join("/");
        return ` / <button onclick="renderBrowser('${fieldId}','${a}')">${p}</button>`;
      })
      .join("");
  ...
  if (!data.dirs.length && !files.length) {
    list = '<div class="fb-empty">Нет файлов</div>';
  }
...
function selectFile(fieldId, filePath, el) {
  ...
  document
    .querySelectorAll(`#fb-${fieldId} .fb-item`)
...
/* ─────────────────────────────────────────────────────────────────────
   СВЕТОФОРЫ (Traffic Lights) — автоматическая реакция на цвет
   ───────────────────────────────────────────────────────────────────── */

const TL_COLOR_NAMES = {
  0: "Чёрный",
  1: "Розовый",
  2: "Фиолетовый",
  3: "Синий",
...
function tlTrainOptions() {
  return (
    [...trainIds]
      .map((id) => {
        const n =
          $(`card-${id}`)?.querySelector(".train-name")?.textContent || id;
        return `<option value="${esc(id)}">${n}</option>`;
      })
      .join("") || '<option value="">— нет поездов —</option>'
  );
}
...
function addTLRule() {
  const colorSel = $("tlRuleColor");
  const speedInp = $("tlRuleSpeed");
  const durInp = $("tlRuleDur");

  const color = parseInt(colorSel.value, 10);
  const speed = Math.max(
    -100,
    Math.min(100, parseInt(speedInp.value, 10) || 0),
  );
  const duration = Math.max(0, parseFloat(durInp.value) || 0) * 1000;

  if (isNaN(color)) return;

  const existing = tlRules.findIndex((r) => r.color === color);
  const rule = { color, colorName: TL_COLOR_NAMES[color], speed, duration };

  if (existing >= 0) tlRules[existing] = rule;
  else tlRules.push(rule);

  renderTLRules();
}

function removeTLRule(color) {
  tlRules = tlRules.filter((r) => r.color !== color);
  renderTLRules();
}

function renderTLRules() {
  const el = $("tlRulesList");
  if (!el) return;

  if (!tlRules.length) {
    el.innerHTML =
      '<div style="color:var(--mut);font-family:var(--fm);font-size:.7rem;padding:6px 0">Правил нет — добавьте выше</div>';
    return;
  }

  el.innerHTML = tlRules
    .map((r) => {
      const dot = `<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${TL_COLOR_CSS[r.color] ?? "#555"};margin-right:4px;vertical-align:middle"></span>`;
      const dur = r.duration > 0 ? `, ${r.duration / 1000} с` : ", ∞";
      const spd =
        r.speed === 0
          ? '<span style="color:var(--red)">СТОП</span>'
          : `<span style="color:var(--grn)">${r.speed}%</span>`;

      return `
      <div class="step-card" style="gap:6px">
        <div style="flex:1;font-family:var(--fm);font-size:.7rem">${dot}${r.colorName} → ${spd}${dur}</div>
        <button class="step-move-btn" onclick="removeTLRule(${r.color})" data-tip="Удалить правило">✕</button>
      </div>`;
    })
    .join("");
}
...
function saveTL() {
  ...
  const trainId = $("tlTrainId").value;
  const port = ($("tlPort")?.value || "B").toUpperCase();

  if (!name || !trainId) return;
  if (!tlRules.length) return;

  const id = tlEditId || `tl${Date.now()}`;

  socket.emit("saveTrafficLight", {
    id,
    tl: { name, trainId, port, active: false, rules: tlRules },
  });
...
function deleteTL(id) {
  socket.emit("deleteTrafficLight", { id });
}

function toggleTL(id, btn) {
  const tl = trafficLights[id];
  if (!tl) return;
  socket.emit("toggleTrafficLight", { id, active: !tl.active });
}
...
function editTL(id) {
  ...
  tlEditId = id;
  tlRules = (tl.rules || []).map((r) => ({ ...r }));

  $("tlName").value = tl.name || "";
  const sel = $("tlTrainId");
  if (sel) sel.value = tl.trainId || "";
  const portSel = $("tlPort");
  if (portSel) portSel.value = tl.port || "B";

  renderTLRules();
  $("tlEditor")?.scrollIntoView({ behavior: "smooth", block: "start" });
}

function renderTrafficLightList() {
  const el = $("tlList");
  if (!el) return;

  const sel = $("tlTrainId");
  if (sel) sel.innerHTML = tlTrainOptions();

  const entries = Object.entries(trafficLights);
  if (!entries.length) {
    el.innerHTML =
      '<div style="color:var(--mut);font-family:var(--fm);font-size:.7rem;padding:8px 0">Светофоров нет</div>';
    return;
  }
  ...
  el.innerHTML = entries
    .map(([id, tl]) => {
      ...
      const trainName =
        $(`card-${tl.trainId}`)?.querySelector(".train-name")?.textContent ||
        tl.trainId?.slice(0, 8) ||
        "—";

      const rulesPreview = (tl.rules || [])
        .map((r) => {
          const dot = `<span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:${TL_COLOR_CSS[r.color] ?? "#555"};margin-right:2px;vertical-align:middle"></span>`;
          return dot + (r.speed === 0 ? "0" : r.speed + "%");
        })
        .join("  ");

      return `
      <div class="consist-list-item" style="flex-wrap:wrap;gap:6px">
        <div style="min-width:70px;font-weight:700">${tl.name}</div>
        <div class="cl-trains" style="flex:1">${trainName} · порт ${tl.port || "B"}</div>
        <div style="font-family:var(--fm);font-size:.62rem;color:var(--mut);width:100%">${rulesPreview}</div>
        <div class="sc-actions" style="width:100%;justify-content:flex-end">
          <button class="sm-btn ${active ? "sm-btn-red" : "sm-btn-grn"}" onclick="toggleTL('${id}',this)">${active ? "■ Стоп" : "▶ Старт"}</button>
          <button class="sm-btn sm-btn-mut" onclick="editTL('${id}')">✏</button>
          <button class="sm-btn sm-btn-mut" onclick="deleteTL('${id}')">🗑</button>
        </div>
      </div>`;
    })
    .join("");
