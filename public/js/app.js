...
function buildSpeedo(id) {
  ...
  [
    ...
    const p = polarSpd(a, S.R + 13),
      anc = a < -20 ? "start" : a > 20 ? "end" : "middle";
...
function updateSpeedo(id, speed) {
  const needle = document.getElementById(`sneedle-${id}`),
    arcEl = document.getElementById(`sarc-${id}`);
  if (!needle) return;
  ...
  const tipX = f(S.CX + tipR * Math.sin(aRad)),
    tipY = f(S.CY - tipR * Math.cos(aRad));
...
socket.on("scenarioRecording", ({ active, name }) => {
  isRecording = active;
  $("recBadge").className = "rec-badge" + (active ? " on" : "");
  $("recBtn").textContent = active ? "■ Стоп" : "⬤ Запись";
  $("recBtn").className = "sm-btn sm-btn-red" + (active ? " active" : "");
});
...
function addTrainCard(d) {
  ...
  $("trainCount").textContent = trainIds.size;
  ...
  card.innerHTML = `
        ...
        <div class="card-inner">
          <div class="card-header">
            ...
            <div class="hdr-btns"><div class="icon-btn settings" onclick="openModal('${esc(id)}')" data-tip="Настройки">🚃</div></div>
          </div>
          ...
          <div class="card-body">
            ...
            <div class="slider-wrap">
              ...
              <div style="position:relative"><input type="range" min="-100" max="100" value="${speed}" step="5" id="slider-${id}"><div class="center-mark"></div></div>
            </div>
            <div class="actions">
              <div class="speed-btns">
                <button class="btn" onclick="setSpeed('${esc(id)}',-100)">◀◀ −100</button>
                <button class="btn" onclick="deltaSpeed('${esc(id)}',-10)">−10</button>
                <button class="btn btn-stop-c" onclick="setSpeed('${esc(id)}',0)">■ СТОП</button>
                <button class="btn" onclick="deltaSpeed('${esc(id)}',10)">+10</button>
                <button class="btn" onclick="setSpeed('${esc(id)}',100)">+100 ▶▶</button>
              </div>
              <div class="start-stop-row">
                <button class="btn btn-go" onclick="doStart('${esc(id)}')">▶ СТАРТ 20%</button>
                <button class="btn btn-halt" onclick="doStop('${esc(id)}')">■ ФИНИШ</button>
              </div>
              <div class="preset-row" id="presets-${id}">${presetsHtml}</div>
            </div>
            <div class="sound-row">
              <button class="btn-snd${sounds.start ? "" : " na"}" onclick="playOnce('${esc(id)}','start')"><span class="ico">🚀</span>СТАРТ</button>
              <button class="btn-snd${sounds.horn ? "" : " na"}" id="horn-${id}" onmousedown="hornOn('${esc(id)}')" onmouseup="hornOff('${esc(id)}')" ontouchstart="hornOn('${esc(id)}')" ontouchend="hornOff('${esc(id)}')"><span class="ico">📯</span>ГУДОК</button>
              <button class="btn-snd${sounds.stop ? "" : " na"}" onclick="playOnce('${esc(id)}','stop')"><span class="ico">🛑</span>ТОРМОЗ</button>
            </div>
          </div>
        </div>`;
...
function toggleMute() {
  ...
  $("muteIco").textContent = globalMuted ? "🔇" : "🔊";
  $("muteLbl").textContent = globalMuted ? "ВЫКЛ" : "Звук";
...
function startLoop(id) {
  ...
  if (s.moving.paused) {
    ...
    const p = s.moving.play().catch(() => {});
    _loopPlayPromise[id] = p;
  }
}
...
function handleSpeedChange(id, newSpeed) {
  ...
  if (prev === 0 && newSpeed !== 0) {
    ...
    const s = trainSounds[id];
    const delay =
      s?.start && isFinite(s.start.duration) && s.start.duration > 0.1
        ? s.start.duration * 1000
        : 900;
    clearTimeout(_startLoopTimer[id]);
    _startLoopTimer[id] = setTimeout(() => {
      startLoop(id);
      setLoopVol(id, prevSpeed[id]);
    }, delay);
...
function deltaSpeed(id, d) {
  const sl = $(`slider-${id}`);
  setSpeed(id, Math.max(-100, Math.min(100, +sl.value + d)));
}
function doStart(id) {
  setSpeed(id, 20);
}
...
function renderConsistList() {
  ...
  el.innerHTML = entries
    .map(
      ([id, c]) =>
        `<div class="consist-list-item"><div style="flex:1;min-width:0"><div class="cl-name">${c.name}</div><div class="cl-trains">${(c.trainIds || []).map((t) => t.slice(0, 8)).join(", ")}</div></div><input type="range" min="-100" max="100" value="${c.speed || 0}" step="5" style="width:90px" oninput="consistSpeedDisplay('${id}',this.value)" onchange="setConsistSpeed('${id}',+this.value)"><span class="cl-spd" id="cspd-${id}">${c.speed || 0}%</span><button class="sm-btn sm-btn-red" onclick="socket.emit('deleteConsist',{id:'${id}'})">🗑</button></div>`,
    )
    .join("");
}
...
/* ── MANUAL SCENARIO BUILDER ────────────────────────────────────
         Структура шага:
           { trainId, trainName, speed, duration, condition }
         condition: null | { type:'colorSensor', port:'B', color:5 }
         Легко расширять: добавить новые type в updateMsCondition() и
         логику обработки в scheduler.js -> _execCondition()
      ─────────────────────────────────────────────────────────────── */
let manualSteps = [];
...
function updateMsCondition() {
  const type = $("msConditionType").value,
    params = $("msConditionParams");
  if (type === "colorSensor") {
    const colorOpts = Object.entries(COLOR_NAMES_RU)
      .map(([v, n]) => `<option value="${v}">${n}</option>`)
      .join("");
    params.innerHTML = `<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
            <div><div class="field-label">Порт сенсора</div>
              <select class="field-input" id="msCondPort"><option>A</option><option selected>B</option><option>C</option><option>D</option></select></div>
            <div><div class="field-label">Цвет</div>
              <select class="field-input" id="msCondColor">${colorOpts}</select></div>
          </div>
          <div style="font-family:var(--fm);font-size:0.61rem;color:var(--mut)">Шаг сработает когда сенсор зафиксирует этот цвет <em>вместо</em> задержки по времени</div>`;
...
/* Загрузить сценарий в построитель (кнопка ✏️ в списке) */
function loadScenarioToBuilder(name) {
  ...
  manualSteps = sc.steps.map((step, i) => {
    ...
    const duration = Math.max(
      0.1,
      Math.round((nextDelay - step.delay) / 100) / 10,
    );
    const trainName =
  ...
  el.innerHTML = `
          ...
          <div class="step-info" style="flex:1;display:grid;gap:5px">
            ...
            <div style="display:grid;grid-template-columns:auto 1fr auto auto;gap:5px;align-items:center">
              ...
              <input type="number" class="field-input" id="es-dur-${i}" min="0.1" max="300" step="0.5"
                value="${step.duration}" style="padding:2px 5px" />
    ...
    manualSteps
      .map((step, i) => {
        ...
        const condHtml = step.condition
          ? `<div class="step-cond">⚡ если цвет сенсора ${step.condition.port} = ${COLOR_NAMES_RU[step.condition.color] || step.condition.color}</div>`
          : "";
      ...
      .join("") +
    `<div style="font-family:var(--fm);font-size:0.63rem;color:var(--mut);padding-top:5px;border-top:1px solid var(--bdr);margin-top:4px">
...
function saveManualScenario() {
  ...
  // Конвертируем в формат планировщика — cumulative delay в мс
  let cumDelay = 0;
  const steps = [];
    ...
    .map(([name, sc]) => {
      ...
      return `<div class="scenario-item">
            <div style="flex:1">
              ...
              <div class="sc-meta" style="display:flex;gap:8px;align-items:center">
            ...
            <div class="sc-actions">
              <button class="sm-btn sm-btn-grn" id="sc-play-${safeId}" onclick="playScenario('${esc(name)}')">▶</button>
              <button class="sm-btn sm-btn-blu" title="Загрузить в редактор" onclick="loadScenarioToBuilder('${esc(name)}')">✏️</button>
              <button class="sm-btn sm-btn-mut" onclick="socket.emit('deleteScenario',{name:'${esc(name)}'})">🗑</button>
            </div>
          </div>`;
    })
    .join("");
...
/* Изменить количество повторов (0 = бесконечно) */
function adjLoops(name, delta) {
  ...
  // Обновляем только значок, не перерисовывая весь список
  const safeId = name.replace(/[\s\W]/g, "_");
  const valEl = $(`sc-loops-${safeId}`);
...
$("schActionType").addEventListener("change", renderSchActionParams);
          ...
          : s.action?.type === "setSpeed"
            ...
            : s.action?.type === "playScenario"
              ...
              : s.action?.type === "setConsist"
                ? `🔗 ${s.action.speed}%`
                : "—";
      return `<div class="sch-item"><span class="sch-time">${s.time}</span><span class="sch-desc">${days} · <b>${s.name}</b> · ${act}</span><button class="sm-btn sm-btn-red" onclick="socket.emit('removeSchedule',{id:'${id}'})">🗑</button></div>`;
    })
    .join("");
}
...
async function openModal(id) {
  ...
  const entry = currentConfig[id] || {},
    sounds = entry.sounds || {},
    presets = entry.presets || [20, 50, 80];
  $("modalTitle").textContent = `Настройки: ${entry.name || id}`;
...
async function saveModal() {
  ...
  const start = $("mStart").value.trim(),
    stop = $("mStop").value.trim();
  const horn = $("mHorn").value.trim(),
    moving = $("mMoving").value.trim();
  const rampStepSize = +$("mRampStep").value,
    rampStepMs = +$("mRampMs").value;
  const presets = [
    +$("mPre0").value || 20,
    +$("mPre1").value || 50,
    +$("mPre2").value || 80,
